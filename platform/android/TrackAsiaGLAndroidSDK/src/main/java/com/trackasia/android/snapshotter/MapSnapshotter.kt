...
open class MapSnapshotter(context: Context, options: Options) {
    ...
    private fun createTextView(mapSnapshot: MapSnapshot, shortText: Boolean, scale: Float): TextView {
        val textColor = ResourcesCompat.getColor(context.resources, R.color.trackasia_gray_dark, context.theme)
        ...
        textView.setBackgroundResource(R.drawable.trackasia_rounded_corner)
    ...
    private fun createScaledLogo(snapshot: Bitmap): Logo {
        val logo = BitmapFactory.decodeResource(context.resources, R.drawable.trackasia_logo_icon, null)
        val scale = calculateLogoScale(snapshot, logo)
        val matrix = Matrix()
        matrix.postScale(scale, scale)
        val helmet = BitmapFactory.decodeResource(context.resources, R.drawable.trackasia_logo_helmet, null)
